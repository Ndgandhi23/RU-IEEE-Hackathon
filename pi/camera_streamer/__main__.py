...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Capture the Pi's USB webcam and serve it as MJPEG to the brain machine.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    cam = Webcam(device=args.device, width=args.width, height=args.height, fps=args.fps)
    try:
        cam.start()
    ...
    while cam.get() is None and time.monotonic() < deadline:
        time.sleep(0.05)
    if cam.get() is None:
        log.warning("no frame after %.1fs warmup; serving anyway, health endpoint will flag it",
                    args.warmup_s)

    server = MjpegServer(cam=cam, host=args.host, port=args.port, jpeg_quality=args.quality)
    try:
        server.start()
    except OSError as e:
